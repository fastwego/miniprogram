...
func TestUpdateOrder(t *testing.T) {
	...
	var resp []byte
	test.MockSvrHandler.HandleFunc(apiUpdateOrder, func(w http.ResponseWriter, r *http.Request) {
		w.Write([]byte(resp))
	})
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			gotResp, err := UpdateOrder(tt.args.ctx, tt.args.payload)
			...
			if !reflect.DeepEqual(gotResp, tt.wantResp) {
				t.Errorf("UpdateOrder() gotResp = %v, want %v", gotResp, tt.wantResp)
			}
		})
	}
}
