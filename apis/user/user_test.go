...
func TestGetPaidUnionId(t *testing.T) {
	...
	var resp []byte
	test.MockSvrHandler.HandleFunc(apiGetPaidUnionId, func(w http.ResponseWriter, r *http.Request) {
		w.Write([]byte(resp))
	})
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			resp = mockResp[tt.name]
			gotResp, err := GetPaidUnionId(tt.args.ctx, tt.args.params)
			//fmt.Println(string(gotResp), err)
			if (err != nil) != tt.wantErr {
			...
			if !reflect.DeepEqual(gotResp, tt.wantResp) {
				t.Errorf("GetPaidUnionId() gotResp = %v, want %v", gotResp, tt.wantResp)
			}
		})
	}
}
