...
func TestAiCrop(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			gotResp, err := AiCrop(tt.args.ctx, tt.args.payload, tt.args.params)
...
func TestScanQRCode(t *testing.T) {
	...
	var resp []byte
	test.MockSvrHandler.HandleFunc(apiScanQRCode, func(w http.ResponseWriter, r *http.Request) {
		w.Write([]byte(resp))
	})
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			resp = mockResp[tt.name]
			gotResp, err := ScanQRCode(tt.args.ctx, tt.args.payload, tt.args.params)
			//fmt.Println(string(gotResp), err)
			if (err != nil) != tt.wantErr {
			...
			if !reflect.DeepEqual(gotResp, tt.wantResp) {
				t.Errorf("ScanQRCode() gotResp = %v, want %v", gotResp, tt.wantResp)
			}
		})
	}
}
func TestSuperresolution(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			resp = mockResp[tt.name]
			gotResp, err := Superresolution(tt.args.ctx, tt.args.payload, tt.args.params)
			//fmt.Println(string(gotResp), err)
			if (err != nil) != tt.wantErr {
