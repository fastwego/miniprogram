...
func TestCheckSessionKey(t *testing.T) {
	...
	var resp []byte
	test.MockSvrHandler.HandleFunc(apiCheckSessionKey, func(w http.ResponseWriter, r *http.Request) {
		w.Write([]byte(resp))
	})
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			gotResp, err := CheckSessionKey(tt.args.ctx, tt.args.params)
			//fmt.Println(string(gotResp), err)
			if (err != nil) != tt.wantErr {
				t.Errorf("CheckSessionKey() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if !reflect.DeepEqual(gotResp, tt.wantResp) {
				t.Errorf("CheckSessionKey() gotResp = %v, want %v", gotResp, tt.wantResp)
			}
		})
	}
}
